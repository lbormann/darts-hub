...
{
    /// <summary>
    /// Factory for creating Pixelit argument controls with enhanced mode support
    /// </summary>
    public static class PixelitArgumentControlFactory
    {
        ...
        public static Control CreateEnhancedArgumentControl(Argument argument, Dictionary<string, Control> argumentControls, 
            Func<Argument, string> getDescription, AppBase pixelitApp)
        {
            var container = new Border
            {
            ...
            // Input Control - Check if this needs enhanced functionality for Pixelit
            Control inputControl;
            if (IsPixelitEffectParameter(argument))
            {
                // For Pixelit, we could add enhanced controls for templates, animations, etc.
                inputControl = CreateEnhancedPixelitControl(argument);
            }
            else
            {
        ...
        private static bool IsPixelitEffectParameter(Argument argument)
        {
            // Check for Pixelit animation/template parameters
            var effectParameters = new[] { "AS", "IDE", "GS", "MS", "G", "M", "B", "HF", "PJ", "PL" };
            return Array.Exists(effectParameters, p => p.Equals(argument.Name, StringComparison.OrdinalIgnoreCase));
        }

        private static Control CreateEnhancedPixelitControl(Argument argument)
        {
            // For now, create a specialized textbox for Pixelit templates/animations
            // This could be enhanced later with template selection dropdowns, etc.
            var enhancedContainer = new StackPanel { Spacing = 5 };
            ...
            var textBox = new TextBox
            {
                ...
                Watermark = GetPixelitPlaceholder(argument)
            };
            ...
            enhancedContainer.Children.Add(textBox);

            // Add helper info for templates/animations
            var helperText = new TextBlock
            {
                Text = GetPixelitHelperText(argument),
                FontSize = 10,
        ...
        private static string GetPixelitPlaceholder(Argument argument)
        {
            return argument.Name.ToUpper() switch
            {
                "AS" => "e.g., startup_animation.json",
                "IDE" => "e.g., idle_display.json", 
                "GS" => "e.g., game_start.json",
                "MS" => "e.g., match_start.json",
                "G" => "e.g., game_won.json",
                "M" => "e.g., match_won.json",
                "B" => "e.g., bust_animation.json",
                "HF" => "e.g., high_finish.json",
                "PJ" => "e.g., player_join.json",
                "PL" => "e.g., player_leave.json",
                _ => "Enter template/animation name"
            };
        }

        private static string GetPixelitHelperText(Argument argument)
        {
            return argument.Name.ToUpper() switch
            {
                "AS" => "Animation shown when application starts",
                "IDE" => "Default display when no game is active",
                "GS" => "Animation when a new game begins",
                "MS" => "Animation when a new match starts", 
                "G" => "Celebration animation for game wins",
                "M" => "Special animation for match victories",
                "B" => "Animation when player goes bust",
                "HF" => "High finish celebration animation",
                "PJ" => "Welcome animation when player joins",
                "PL" => "Farewell animation when player leaves",
                _ => "Pixelit template or animation file"
            };
        }
