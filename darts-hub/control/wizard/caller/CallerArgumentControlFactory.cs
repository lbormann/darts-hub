...
{
    /// <summary>
    /// Factory for creating Caller argument controls with enhanced mode support
    /// </summary>
    public static class CallerArgumentControlFactory
    {
        ...
        public static Control CreateEnhancedArgumentControl(Argument argument, Dictionary<string, Control> argumentControls, 
            Func<Argument, string> getDescription, AppBase callerApp)
        {
            var container = new Border
            {
            ...
            // Input Control - Check if this needs enhanced functionality for Caller
            Control inputControl;
            if (IsCallerHardwareParameter(argument))
            {
                // For hardware/camera parameters, add enhanced controls
                inputControl = CreateEnhancedCallerControl(argument);
            }
            else
            {
                // Use basic control for non-hardware parameters
        ...
        private static bool IsCallerHardwareParameter(Argument argument)
        {
            // Check for caller media path parameters instead of hardware
            var mediaParameters = new[] { "M", "MS", "MEDIA_PATH", "MEDIA_SHARED" };
            return Array.Exists(mediaParameters, p => p.Equals(argument.Name, StringComparison.OrdinalIgnoreCase)) ||
                   ...
                   argument.NameHuman.ToLower().Contains("media");
        }

        private static Control CreateEnhancedCallerControl(Argument argument)
        {
            // For media path parameters, create enhanced path selection
            ...
            var textBox = new TextBox
            {
                ...
                Width = 280,
                Watermark = GetCallerPlaceholder(argument)
            };
            ...
            {
                Text = GetCallerHelperText(argument),
                FontSize = 10,
        ...
        private static string GetCallerPlaceholder(Argument argument)
        {
            return argument.Name.ToUpper() switch
            {
                "M" => "e.g., C:\\Games\\Darts\\Media",
                "MS" => "e.g., C:\\Games\\Darts\\SharedMedia", 
                "U" => "e.g., your.email@example.com",
                "B" => "e.g., your-board-id-123",
                "HP" => "e.g., 8079",
                _ => "Enter value"
            };
        }

        private static string GetCallerHelperText(Argument argument)
        {
            return argument.Name.ToUpper() switch
            {
                "M" => "Folder containing voice announcement files and media for the caller",
                "MS" => "Optional shared media folder for additional voice packs and announcements",
                "U" => "Your Autodarts account email address for authentication",
                "P" => "Your Autodarts account password (stored securely)",
                "B" => "Your unique Autodarts board identifier from your board settings",
                "HP" => "Port number for caller web service (default: 8079)",
                _ => "Darts caller voice announcement parameter"
            };
        }
        ...
        private static Control CreateNumericUpDown(Argument argument, bool isFloat)
        {
            ...
            if (ArgumentTypeHelper.TryGetNumericRange(argument, out var minimum, out var maximum))
            {
                ...
                System.Diagnostics.Debug.WriteLine($"[CallerArgFactory] Applied type-based range constraints to {argument.Name}: Min={minimum}, Max={maximum}");
            }
            else
            {
        ...
        private static void SetNamedParameterLimits(NumericUpDown control, Argument argument, bool isFloat)
        {
            ...
            switch (argName)
            {
                ...
                default:
                    ...
                    System.Diagnostics.Debug.WriteLine($"[CallerArgFactory] Using default positive range for {argument.Name}: Min={control.Minimum}, Max={control.Maximum}");
                    break;
            }
        }
